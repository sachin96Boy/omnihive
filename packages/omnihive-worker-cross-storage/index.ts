...
import { IStorageWorker } from "@withonevision/omnihive-public-queen/interfaces/IStorageWorker";
        ...
        const promise: Promise<boolean> = new Promise<boolean>((resolve, _reject) => {
            ...
            this.storageClient.onConnect()
                .then(() => {
                    ...
                    this.storageClient.get(`${this.metadata.keyPrefix}::${key}`)
        ...
        const promise: Promise<boolean> = new Promise<boolean>((resolve, _reject) => {
            ...
            this.storageClient.onConnect()
                .then(() => {
                    ...
                    this.storageClient.del(`${this.metadata.keyPrefix}::${key}`)
                        .then(() => resolve(true))
                        .catch(() => resolve(false));
                })
                .catch(() => {
        ...
        const promise: Promise<boolean> = new Promise<boolean>((resolve, _reject) => {
            ...
            this.storageClient.onConnect()
                .then(() => {
                    ...
                    // Set the item
                    this.storageClient.set(`${this.metadata.keyPrefix}::${key}`, encrypted)
                        .then(() => resolve(true))
                        .catch(() => resolve(false));
                })
                .catch(() => {
