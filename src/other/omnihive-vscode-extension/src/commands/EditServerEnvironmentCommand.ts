...
export class EditServerEnvironmentCommand {
    public setup = (context: vscode.ExtensionContext) => {
        const cmdOhEditServerEnvironment = vscode.commands.registerCommand(VsCodeCommand.EditServerEnvironment, (args: any) => {
            ...
            panel.webview.onDidReceiveMessage((message: VsCodePostMessageModel) => {
                if (message.command === VsCodeCommand.EditServerEnvironment && message.data && message.data.settings) {
