...
export const SettingsEditor: React.FC<Props> = (props): React.ReactElement => {
    ...
    const [workerEditorObject, setWorkerEditorObject] = React.useState<WorkerEditorObject | undefined>(undefined);

    const server = props.panelProps.registeredServers.filter(
    ...
    React.useEffect(() => {
        ...
        socket.on(
            ...
            (message: AdminResponse<{ config: ServerConfig; systemEnvironmentVariables: EnvironmentVariable[] }>) => {
                ...
                if (IsHelper.isNullOrUndefined(message.data)) {
                    return;
                }
                ...
                if (IsHelper.isNullOrUndefined(message) || !message.requestComplete || !message.data.config) {
                    if (!IsHelper.isNullOrUndefined(message.requestError)) {
        ...
        socket.on(AdminEventType.ConfigSaveResponse, (message: AdminResponse<{ verified: boolean }>) => {
            ...
            setTimeout(() => {
                const postMessage: VsCodePostMessageModel = {
                    command: codeCommand ?? VsCodeCommand.EditServerEnvironment,
                    data: {
    ...
    const addButtonHandler = (addType: string) => {
        ...
        let newEnvrionmentVariableType: EnvironmentVariableType = EnvironmentVariableType.String;
    ...
    const gridSelection = () => {
        if (IsHelper.isNullOrUndefined(gridApi)) {
            return;
        }
        ...
        <>
            ...
            {showEditor && props.settingsSection === "workers" && (
                <WorkerModal
                    ...
                    onCancel={() => setShowEditor(false)}
                    onDelete={(indexValue: number) => deleteHandler(indexValue)}
                    editorObject={workerEditorObject ?? { index: -1, ...new HiveWorkerConfig() }}
