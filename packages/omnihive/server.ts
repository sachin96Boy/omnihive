...
const start = async (): Promise<void> => {
    ...
    const adminPubSubClient: IPubSubClientWorker | undefined = await AwaitHelper.execute<IPubSubClientWorker | undefined>(
        CommonStore.getInstance().getHiveWorker<IPubSubClientWorker>(HiveWorkerType.PubSubClient, OmniHiveConstants.ADMIN_PUBSUB_CLIENT_WORKER_INSTANCE));


    adminPubSubClient?.joinChannel(CommonStore.getInstance().settings.server.serverGroupName);
    logWorker.write(OmniHiveLogLevel.Info, "Admin Pusher Channel => Connected");

    adminPubSubClient?.addListener(CommonStore.getInstance().settings.server.serverGroupName, "server-reset-request", (data: { reset: boolean; }) => {
                    ...
                    buildServer()
                        .then(() => {
                            ...
                            adminPubSubServer?.emit(
                                CommonStore.getInstance().settings.server.serverGroupName,
                                "server-reset-result",
                                {
    ...
    try {
        ...
        for (const builder of buildWorkers) {
            ...
            if (builderDbWorkers.length > 0) {
                ...
                const graphDatabaseConfig: ApolloServerExpressConfig = {
                    ...
                    tracing: CommonStore.getInstance().settings.server.developerMode,
                    context: async ({ req }) => {
                        const tokens = {
                ...
                if (CommonStore.getInstance().settings.server.enableGraphPlayground) {
                    graphDatabaseConfig.introspection = true;
                    graphDatabaseConfig.playground = {
                        endpoint: `${CommonStore.getInstance().settings.server.rootUrl}/graphql/database${builderMeta.graphUrl}`
                    };
                } else {
        ...
        if (CommonStore.getInstance().workers.some((worker: [HiveWorker, any]) => worker[0].type === HiveWorkerType.GraphEndpointFunction && worker[0].enabled === true)) {
            ...
            if (CommonStore.getInstance().settings.server.enableGraphPlayground) {
                ...
                graphFunctionConfig.playground = {
                    endpoint: `${CommonStore.getInstance().settings.server.rootUrl}/graphql/custom`
                };
            } else {
        ...
        if (CommonStore.getInstance().workers.some((worker: [HiveWorker, any]) => worker[0].type === HiveWorkerType.RestEndpointFunction && worker[0].enabled === true)) {
            ...
            const swaggerDefinition: swaggerUi.JsonObject = {
                ...
                servers: [
                    {
                        url: `${CommonStore.getInstance().settings.server.rootUrl}${OmniHiveConstants.CUSTOM_REST_ROOT}`,
                    },
                ],
            };
