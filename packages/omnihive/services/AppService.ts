...
export class AppService {
    ...
    public initApp = async (settingsFile: string) => {
        ...
        try {
            const settingsJson: ServerSettings = ObjectHelper.createStrict(ServerSettings, JSON.parse(fileSystemWorker.readFile(`${settingsFile}`)));
            CommonStore.getInstance().settings = settingsJson;
        } catch {
            throw new Error("No valid configuration files found that can be parsed into server settings");
        }
